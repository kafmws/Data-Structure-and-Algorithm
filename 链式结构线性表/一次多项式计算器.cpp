...
void multiply(List *list, List list1, List list2) {
	...
	while (p) {
		...
		i++; p = p->next; q = list2.head->next;
	}
	for (i = 1; i < size; i++) {
		add(&tem[0], tem[0], tem[1]);
	}
	list->head = tem->head;
	list->tail = list->tail;
}
...
int main() {
	...
	switch (choice) {
	case 1:print(list[2]); break;
	case 2:minus(&list[3], list[0], list[1]);print(list[3]); break;
	case 3:multiply(&list[6], list[0], list[1]); print(list[6]); break;
	case 4:print(list[5]); break;
	}
	system("pause");
}
